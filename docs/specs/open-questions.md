...
| **Whether a box drag ranks what it covers**<br>(it does not: `crates/editor/src/selection.rs` says only the boundary between gestures is meaningful) | after the inspector can commit an edit | when an edit committed through the inspector lands on an entity the user did not mean, which cannot happen until a field row can be written back | whether "the last one chosen" should mean something inside a boxed group, and what would rank them: the order the world iterates, where the drag ended, or the draw order. **The header half of this was answered** and is no longer open: it reads `1 of 3 selected` ([ui.md §6](./ui.md)), which makes the arbitrary choice visible rather than better |
