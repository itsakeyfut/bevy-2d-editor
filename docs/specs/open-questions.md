...
* ~~Whether a box drag ranks what it covers~~ → **it is to, front to back, so that the last element is the entity in front** ([ui.md §4](./ui.md)). Its trigger condition was an edit that could land on an entity the user did not mean, and [ui.md §7](./ui.md) is that edit. The code followed in issue #47; a tie in depth keeps the world's order and is still arbitrary, which the header's count in [ui.md §6](./ui.md) keeps visible
...
| **Undoing a keystroke at a time inside a text box**<br>(the whole of what was typed, in [ui.md §8](./ui.md)) | when Bevy ships it | when `bevy_text`'s `EditableText` implements undo, which its module documentation lists as planned and not implemented in 0.19.1 | whether its binding stops Ctrl+Z, or the redo keys of [ui.md §9](./ui.md), propagating, which the tests named in those sections would report. Whether taking back all of what was typed turned out to be enough in use |
