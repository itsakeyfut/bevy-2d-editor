...
| **Undoing a keystroke at a time inside a text box**<br>(the whole of what was typed, in [ui.md §8](./ui.md)) | when Bevy ships it | when `bevy_text`'s `EditableText` implements undo, which its module documentation lists as planned and not implemented in 0.19.1 | whether its binding stops Ctrl+Z propagating, which the test named in [ui.md §8](./ui.md) would report. Whether taking back all of what was typed turned out to be enough in use |
