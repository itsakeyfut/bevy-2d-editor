...
| **What the inspector's header names after a box drag**<br>(an arbitrary member of what the box covered, today) | when [ui.md §6](./ui.md)'s header carries a value rather than a name, which is the issue after the one that built it | when the panel shows what is *in* a component, so that naming the wrong member of a boxed group stops being cosmetic and starts being an edit aimed at the wrong entity | whether [ui.md §4](./ui.md)'s box drag should rank what it covers after all, which `crates/editor/src/selection.rs` currently declines to do, or whether the header should say how many are selected instead of naming one |
